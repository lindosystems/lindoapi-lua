...
local logger = Logclass_:new('logger','info')
...
local error_objects = {
  ...
  method_failed     = { err_code=-32604, err_message="RPC Method failed"},
...
local function response_error(req, error_name, data)
  local res = {}
  --logger.warn(">> error_name:%s\n",error_name)
  --print_table3(get_error_object(tostring(error_name)))
  local error_object = get_error_object(tostring(error_name)) or
...
local function handle_request(methods, req)
  ...
  if (params=='null') or (params==nil) then
    params = {}
  end
...
local function server_response(methods, request)
  ...
  if type(request) == 'string' then
    ...
    local status = req and true or false
